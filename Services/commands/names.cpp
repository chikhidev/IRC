...
void Services::names(Client &client, std::vector<std::string> &params)
{
    ...
    if (params.empty())
    {
        server->dmClient(client, ERR_NEEDMOREPARAMS, client.getNick() + " NAMES :Not enough parameters");
        return;
    }
    ...
    if (channel_name[0] != '#')
    {
        ...
        server->dmClient(client, RPL_ENDNAMES, client.getNick() + " " + channel_name + " :End of /NAMES list");
        return;
    }
    ...
    {
        server->dmClient(client, ERR_NOTONCHANNEL, client.getNick() + " " + channel_name + " :You're not on that channel");
        server->dmClient(client, RPL_ENDNAMES, client.getNick() + " " + channel_name + " :End of /NAMES list");
        return;
    }
